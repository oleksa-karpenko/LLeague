    ...
    <section className="rise">
      ...
      <form className="toolbar" onSubmit={onSubmit}>
        ...
        <button
          ...
          type="submit"
          disabled={!eventId || create.isPending || !name.trim()}
          title={!eventId ? 'Pick an event first' : undefined}
        >
      ...
      {!eventId && (
        <p className="dim" style={{ marginTop: -8, marginBottom: 14, fontSize: '0.82rem' }}>
          Select a specific event above to add a division to it.
        </p>
      )}
      ...
      {isLoading ? (
        ...
        <div className="table-wrap">
          <EmptyState
            ...
            title="No divisions"
            hint="Divisions group teams, tables and matches inside an event."
          />
        </div>
      ) : (
        <div className="table-wrap">
                ...
                <tr key={d.id}>
                  <td>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 9 }}>
                      <span
                        className="swatch"
                        style={{ background: d.color, width: 16, height: 16 }}
                      />
                  ...
                  <td>
                    <div className="row" style={{ gap: 6 }}>
                      <Link className="btn btn-sm" to={`/division/${d.id}/setup`}>
                        <IconWrench size={14} /> Setup
                      </Link>
                      <Link className="btn btn-sm" to={`/division/${d.id}/score`}>
                        <IconClipboard size={14} /> Score
                      </Link>
                      <Link className="btn btn-sm" to={`/division/${d.id}/board`}>
                        <IconFlag size={14} /> Board
                      </Link>
                    </div>
                  </td>
