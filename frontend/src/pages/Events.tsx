...
export default function Events() {
  ...
  const create = useMutation({
    mutationFn: () => createEvent({ seasonId, name: name.trim(), slug: '', startDate, endDate, location: location.trim() }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['events', seasonId] }); setName(''); setLocation(''); },
  });
    ...
    <section className="rise">
      ...
      <form className="toolbar" onSubmit={onSubmit}>
        <div className="field" style={{ flex: '1 1 200px' }}>
          ...
          <input className="input" placeholder="Regional Championship" value={name} onChange={(e) => setName(e.target.value)} />
        </div>
        <div className="field" style={{ flex: '1 1 160px' }}>
          <label className="label">Location</label>
          <input className="input" placeholder="City, venue" value={location} onChange={(e) => setLocation(e.target.value)} />
        </div>
        <div className="field">
        ...
        <button className="btn btn-primary" type="submit" disabled={!seasonId || create.isPending || !name.trim()} title={!seasonId ? 'Pick a season first' : undefined}>
          <IconPlus size={16} /> Add event
        </button>
      </form>
      {!seasonId && <p className="dim" style={{ marginTop: -8, marginBottom: 14, fontSize: '0.82rem' }}>Select a specific season above to add an event to it.</p>}
      {create.error && <ErrorBanner>{(create.error as Error).message}</ErrorBanner>}

      {isLoading ? <Loading />
        : error ? <ErrorBanner>{(error as Error).message}</ErrorBanner>
        : events.length === 0 ? (
          <div className="table-wrap"><EmptyState icon={<IconFlag size={22} />} title="No events" hint="Events belong to a season — pick one above and add it." /></div>
        ) : (
          <div className="table-wrap">
            <table className="data">
              <thead><tr><th>Event</th><th>Dates</th><th>Location</th><th /></tr></thead>
              <tbody>
                {events.map((ev) => (
                  <tr key={ev.id}>
                    <td className="cell-strong">{ev.name}</td>
                    <td className="mono muted">{ev.startDate}<span className="dim"> → </span>{ev.endDate}</td>
                    <td className="muted">{ev.location || '—'}</td>
                    <td className="actions">
                      <button className="btn btn-danger btn-sm" onClick={() => remove.mutate(ev.id)} disabled={remove.isPending}>
